...
pushOptions := new OptionTable from {
	  ...
	  DegreeLimit => {},
	  PairLimit => infinity,
	  GetMaps => false, -- return map from original module to pushed-forward module, and basis of original module
	  -- unused options:
...
pushNonLinear := opts -> (f,M) -> (				    -- this returns the presentation matrix of the pushforward module
	...
	if opts.GetMaps then (
	     mgens := target m;
	     Mtom := map(mgens, M, id_M);
	     m1gens := target m1;
	     Mtom1 := map(m1gens, M, xvars, (xvars Mtom) ** id_((ring gensJ)^1));
	     )
    ...
    then f.cache#comp#0
	 ...
	 if opts.GetMaps then (
	      pfM := cokernel result;
	      m1topfM := map(pfM, m1gens, mapback, id_pfM);
	      MtopfM := m1topfM * Mtom1;
	      result, MtopfM
	      )
...
pushForward(RingMap, Module) := Module => opxots -> (f,M) -> (
     if isHomogeneous f and isHomogeneous M then (
	  ...
	  --next line only works if GetMaps is set
	  (pfM, pfmap) := (pushNonLinear opts)(f,M2);
